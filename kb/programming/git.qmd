...
[git](https://git-scm.com/) is a software version control system. git is useful for tracking changes to code, including systematic approaches to adding new features or changing functionality. We use git to track and save code as we're working on it.

[GitHub](https://github.com/) is one of several online repositories for sharing code (and is commonly used as a remote repository for git). We use GitHub to facilitate collaboration and share code for papers and other projects.
...
When you start a new project, you should [create a GitHub meta-repository](https://docs.github.com/en/github/getting-started-with-github/create-a-repo) right away. You can do this in your personal account or as part of the `srikrishnan-lab` organization. Make sure that your repository name is informative; we will eventually rename this to align with a paper (something like `lead_author-etal-year`).

A meta-repository is a repository which creates a single point of access for all resources needed to create a published work. We use meta-repositories to avoid mixing data, models, and analysis codes, which can inhibit reproducibility, make the repository cluttered, and make it difficult to re-use models for subsequent projects. You can fork [our meta-repository template](https://github.com/srikrishnan-lab/paper-metarepo-template) as a starting point. There is a companion [model repository template](https://github.com/srikrishnan-lab/model_repo_template) for model codes, which live in their own repository rather than inside the meta-repository. [GitHub and Zenodo](../../guides/github-zenodo.qmd) explains why we split them and how both get archived before submission.
...
There are several alternatives to GitHub, including [Bitbucket](https://bitbucket.org/). You may use whichever online repository you prefer, though we use GitHub for lab-wide management, such as the website and lab manual.

## Learn More

* [Version Control with Git](http://swcarpentry.github.io/git-novice/) from Software Carpentry
* [GitHub Skills](https://skills.github.com/) from GitHub.
* [Getting Started: Git and GitHub](https://waterprogramming.wordpress.com/2014/09/29/getting-started-git-and-github/) from Water Programming.
* A [collection of git-related posts](https://waterprogramming.wordpress.com/category/programming/git-and-github/) from Water Programming.

## When You're Stuck

Git's error messages are famously unhelpful, and most problems are one of a small number of situations.

1. **Stop and look before acting.** `git status`, then `git log --oneline --graph --all`. Most confusion is not knowing which branch you are on or what is staged.
2. **[Oh Shit, Git!?!](https://ohshitgit.com/)** ([polite version](https://dangitgit.com/)) — organized by the situation you are in rather than by command, which is the right index for git.
3. **Almost nothing is unrecoverable.** `git reflog` shows where HEAD has been, including commits you think you destroyed. Before a scary operation, note the current SHA.
4. **[Version Control with Git](https://swcarpentry.github.io/git-novice/)** if the gap is conceptual.
5. **Ask before force-pushing a shared branch.** This is the one class of git mistake that damages other people's work rather than just your own. On anything in the `srikrishnan-lab` organization, ask first.
